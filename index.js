...
app.post("/signup", async function (req, res) {
  try {
    ...
    connection.then(async () => {
      ...
      if (
        user != undefined ||
        (user != null && user.userID == req.body.signupemail)
      ) {
  ...
  try {
    ...
    connection.then(async () => {
      ...
      if (user == undefined || user == null) {
        ...
        // Perform actions on the collection object
        if (await bcrypt.compare(req.body.loginpassword, user.userPassword)) {
...
app.post("/addmatchdata", authenticateToken, function (req, res) {
  try {
    ...
    connection.then(async () => {
      ...
      if (exists) {
        //Update old score of same match
        collection
          .updateOne(
            {
              userID: req.user.ID,
...
function authenticateToken(req, res, next) {
  ...
  if (token == null)
    return res
      .status(401)
      .sendFile(path.resolve(__dirname + "/public/forbidden.html"));
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, user) => {
    if (err) {
      ...
      return res
        .status(403)
        .sendFile(path.resolve(__dirname + "/public/expired.html"));
    }
    req.user = user;
    next();
  });
...
app.post("/roomlength", (req, res) => {
  ...
  var roomSize = io.sockets.adapter.rooms.get(req.body.ID).size;
  console.log("RoomID:", roomID);
  if (roomID && roomSize < 2) res.send("Notfull");
  else if (roomID && roomSize >= 2) res.send("Full");
...
io.on("connection", (socket) => {
  ...
  socket.on("createNewGame", (data) => {
    ...
    connection.then(async () => {
      ...
      var user = await collection
        .findOne({
          _id: new ObjectId("5ea802a35a6c8f21c04c254b"),
        })
        .then((result) => {
          ...
          collection
            .updateOne(
              {
                _id: new ObjectId("5ea802a35a6c8f21c04c254b"), //Query
              },
              {
                $set: {
